...
void main() {
  const String testUri = 'https://api.themoviedb.org/3/movie/upcoming?api_key=a64533e7ece6c72731da47c9c8bc691f&language=ko-KR&page=1';

  test('실제 TMDB API를 호출', () async {
  ...
  test('Mock 데이터 호출', () async {
    final MovieRepository movieRepository = MovieRepositoryImpl(
      movieDataSource: MovieDataSourceImpl(
        client: MockHttpClientImpl().createMovieMockClient(testUri),
        uri: testUri,
      ),
    );
    List<Movie> movies = await movieRepository.getMovieInfoList();
