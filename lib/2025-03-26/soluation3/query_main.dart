...
void main() {
  ...
  print("1. ${transactList1.map((transact) => transact.trader.name).toList()}");
  ...
  print(
    "3. ${cambridgeTraderList.map((trader) => {"name": trader.name, "cit"
        "y": trader.city})}",
  );

  //4.모든 거래자의 이름을 알파벳순으로 정렬해서 나열하시오
  List<String> traderNameSortList =
      transactions.map((transaction) => transaction.trader.name).toList();
  traderNameSortList.sort((a, b) => a.compareTo(b));
  print("4. ${traderNameSortList.toSet()}");
